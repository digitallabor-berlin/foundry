# AGENTS.md — `crates/oid4vci` (VENDORED)

## ⚠️ This is a vendored owned copy, not upstream code you should refactor

Vendored from [`spruceid/oid4vci-rs`](https://github.com/spruceid/oid4vci-rs) at
commit `7cfd5a012229de08aac363b27a41e226b494f9cc` (2026-07-17). It provides the
OpenID4VCI protocol baseline models used by `foundry-issuer`.
...
2. **Prefer wrapping over editing.** If you need different behaviour, implement
   it in `foundry-issuer` or `foundry-core` and leave the vendored code alone.
...
6. `[[example]]` entries were removed during vendoring because `examples/` was
   not copied — do not re-add them.
