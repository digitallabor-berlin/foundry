...
| `paso_metadata.rs` | `GET /credential-metadata/:id`: `Accept` negotiation between the bare JSON object and the signed JWT, absent-`Accept` default, 406 on an unsatisfiable `Accept`, 404 for both unknown and configured-but-non-PaSO ids, the Issuer Metadata advertisement, and the PaSO Proof Metadata §7 wallet-side verification of a JWT fetched over HTTP (signature against its own `x5c` leaf, with a tampered-payload negative control) plus §8's URI binding | `server::credential_metadata_handler`, `negotiate_metadata_representation`, `foundry_issuer::build_credential_metadata_jwt` |
| `paso_adhoc_metadata.rs` | `POST /admin/paso/ad-hoc-metadata`: the configured-metadata default path, an override introducing a type absent from config (§5.4), 400 for an unknown credential type / an unconfigured type with no override / a structurally invalid override, explicit `ttl_secs`, and that the route sits behind the API key | `server::create_adhoc_metadata_handler`, `foundry_issuer::build_adhoc_metadata_jwt`, `require_api_key` |
...
| `docs_hygiene.rs` | Every markdown page MkDocs builds links into `docs/specs/` and `docs/superpowers/` only by absolute URL. Those trees are excluded from the site via `exclude_docs`, and MkDocs caps such a link's log level at INFO, so `mkdocs build --strict` cannot catch it and no config key promotes it — verified in both directions when the test was written. Root `AGENTS.md` §4.4 makes spec citation normative, so this is the enforcement mechanism | the built `docs/` tree; no crate code |
