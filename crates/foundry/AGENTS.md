...
| `/admin/paso/ad-hoc-metadata` | POST | **Bearer** | `create_adhoc_metadata_handler` → `foundry_issuer::build_adhoc_metadata_jwt` — mints an `adhoc-transaction-metadata+jwt` (PaSO Proof Metadata §5.2) for a Relying Party to embed in a `transaction_data` entry. An operator API precisely because §5.1 leaves "the mechanism by which the Relying Party obtains it from the Attestation Provider" out of scope. Logs the credential type and transaction data type, but only the **presence** of a metadata override — its contents can carry payee names and amount formats |
...
| `/credential-offer/:id` | GET | `foundry_issuer::load_offer_by_reference` — the Credential Offer Object served **by reference** (OpenID4VCI §4.2, L432); `application/json` per L445, 404 for an unknown/expired id. **Unconditionally registered** even when `issuer.offer_by_reference` is off, unlike `/challenge`: it has no metadata twin to agree with, and always serving it means flipping the flag off cannot strand offers already in flight. The `:id` is a bearer credential (the body carries the `pre-authorized_code`) — never logged |
| `/credential-metadata/:credential_configuration_id` | GET | `foundry_issuer::build_credential_metadata_document` or `build_credential_metadata_jwt` — PaSO Proof Metadata §2, content-negotiated on `Accept` between the bare `credential_metadata` object (`application/json`, also the default when `Accept` is absent or a wildcard) and the signed `credential-metadata+jwt` of §4 (`application/jwt`). **The only route in this crate that reads an `Accept` header**; the negotiation is a separately unit-tested pure function, `negotiate_metadata_representation`. 406 when `Accept` names neither; 404 for an unknown id **and** for a configured type that is not a PaSO Credential type, since §3 makes `transaction_data_types` REQUIRED in what this serves. **Unconditionally registered** — the handler's 404 is what keeps the route's existence from contradicting Issuer Metadata |
