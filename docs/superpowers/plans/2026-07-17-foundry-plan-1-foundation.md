...
- [ ] **Step 4: Full workspace verification (fmt, clippy, tests)**
...
```
Expected: fmt clean, clippy no warnings, all tests pass across `foundry-core` and `foundry`. (Vendored `oid4vci`/`openid4vp` tests may be excluded if upstream tests need network — if so, run `cargo test --workspace --exclude oid4vci --exclude openid4vp` and note it.)

- [ ] **Step 5: Commit**

```bash
git add crates/foundry
git commit -m "feat(server): add background expiry sweeper and pass full workspace check"
```
