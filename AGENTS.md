...
| [`draft-ietf-oauth-status-list-14.txt`](docs/specs/draft-ietf-oauth-status-list-14.txt) | Token Status List (TSL) — the revocation/suspension mechanism HAIP L326 mandates by reference ("The `status` claim, if present, MUST contain `status_list`"): `foundry-core`'s `status_list/` (bit-packing, zlib, Status List Token build/sign/verify), the `GET /statuslists/:id` route, `foundry-issuer`'s `status_index.rs` and the `status` claim on issued credentials, and `foundry-verifier`'s `status.rs` check. Where HAIP is stricter it wins — notably HAIP L327 requires the verification key in the Status List Token's `x5c` and forbids the trust anchor there, which this draft leaves open (§11.3 mandates no key-resolution method at all). Kept as `.txt`, not `.md` — verbatim fidelity to the IETF text is the point of a pinned draft. **Interop constraint, not a spec requirement:** §11.3 permits the Status List Token to be signed by a key other than the credential issuer's, and §13.5 permits a wholly separate Status Issuer, but the Credo / `@sd-jwt` wallet stack verifies the Status List Token with the *credential issuer's* key and never reads the token's own `x5c` — so a divergent signer is unverifiable in those wallets. `Config::validate()` warns; see `crates/foundry-core/AGENTS.md` Gotchas |
